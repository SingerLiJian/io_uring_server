...
void add_log(struct io_uring *ring, int fd, unsigned flags, char* str)  ;
...
int main(int argc, char *argv[]) {
    ...
    int m_Fd = open(LOG_FILE_NAME, O_RDWR | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR); 
    char conn[50];
    // char read[20] = "read success";
    // char write[20] = "write success";


    if (argc < 2) {
        printf("Please give a port number: ./my_echo_server [port]\n");
        exit(0);
    }
    ...
    printf("my echo server listening for connections on port: %d\n", portno);
    ...
    while (1) {
        ...
        io_uring_for_each_cqe(&ring, head, cqe) {
            ...
            if (cqe->res == -ENOBUFS) {
                ...
                if ( sock_conn_fd > 0 ) {
                    //get addr and port of client
                    getpeername(sock_conn_fd,(struct sockaddr*)&client_addr,&client_len);
                    char ip[INET_ADDRSTRLEN];
                    inet_ntop(AF_INET,&client_addr.sin_addr,ip,sizeof(ip));
                    int port = ntohs(client_addr.sin_port);
                    snprintf(conn, 50, "connected the client: %s:%d\n" , ip, port);
                    add_log(&ring, m_Fd, 0, conn);
...
void add_log(struct io_uring *ring, int fd, unsigned flags, char* str){
    ...
    off_t offset=0;
    io_uring_prep_write(sqe, fd, str, bytes, offset);
